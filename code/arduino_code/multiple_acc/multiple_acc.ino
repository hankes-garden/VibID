...
accel_t_gyro_union g_data0;
accel_t_gyro_union g_data1;
...
{      
  ...
  // // error = MPU6050_read (MPU6050_I2C_ADDRESS_0, MPU6050_WHO_AM_I, &c, 1);
  ...
  // error = MPU6050_read (MPU6050_I2C_ADDRESS_0, MPU6050_PWR_MGMT_1, &c, 1);
...
void loop()
{
  ...
  uint8_t swapBuffer;
  #define SWAP(x,y) swapBuffer = x; x = y; y = swapBuffer

  SWAP (g_data0.reg.x_accel_h, g_data0.reg.x_accel_l);
  SWAP (g_data0.reg.y_accel_h, g_data0.reg.y_accel_l);
  SWAP (g_data0.reg.z_accel_h, g_data0.reg.z_accel_l);
  SWAP (g_data0.reg.t_h, g_data0.reg.t_l);
  SWAP (g_data0.reg.x_gyro_h, g_data0.reg.x_gyro_l);
  ...
  // Serial.print(millis());
  // Serial.print(F(","));
  Serial.print(g_data0.value.x_accel, DEC);
  Serial.print(F(", "));
  ...
  // dT = ( (double) g_data0.value.temperature + 12412.0) / 340.0;
  // Serial.print(dT, 3);
  ...
  // Serial.print(g_data0.value.x_gyro, DEC);
  // Serial.print(F(", "));
  // Serial.print(g_data0.value.y_gyro, DEC);
  // Serial.print(F(", "));
  // Serial.print(g_data0.value.z_gyro, DEC);
  // Serial.print(F(", "));
  // Serial.println(F(""));
}
...
//
int MPU6050_read(uint8_t addr, int start, uint8_t *buffer, int size)
{
  int i, n, error;
...
//
int MPU6050_write(uint8_t addr, int start, const uint8_t *pData, int size)
{
  int n, error;
...
//
int MPU6050_write_reg(uint8_t addr, int reg, uint8_t data)
{
  int error;
